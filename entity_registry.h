...
public:
	...
	void destroy_entity(uint64_t entity){
		...
		//LOG_ERROR("[EntityRegistry] Could not destroy entity with id: {}. It does not exist.", entity);
	}
	...
	T& assign_component(uint64_t entity){
		...
		//LOG_ERROR("[EntityRegistry] Could not add component to entity with id: {}. it does not exist.", entity);
	}
	...
	void remove_component(uint64_t entity){
		...
		//LOG_ERROR("[EntityRegistry] Could not remove component from entity with id: {}. It does not exists.", entity);
	}
	...
	T& get_component(uint64_t entity){
		...
		if (it != _entities.end()){
			...
			{
				//LOG_ERROR("[EntityRegistry] Could not get component for entity with id: {}. Entity does not have this component.", entity);
			}
			auto component = static_cast<T*>(_component_pools[component_id]->get_data(entity));
			return *component;
		}

		//LOG_ERROR("[EntityRegistry] Could not get component for entity with id: {}. Entity does exists.", entity);
	}
	...
	void add_entity_to_group(uint64_t entity, const std::string& group_name){
		...
		//LOG_ERROR("[EntityRegistry] Could not add entity with id: {} to group: {}. Entity does not exists.", entity, group_name);
	}

	void remove_entity_from_group(uint64_t entity, const std::string& group_name){
		if (!_entity_groups.contains(group_name)){
			//LOG_ERROR("[EntityRegistry] Could not remove entity with id: {} to group {}. Group does not exists.", entity, group_name);
			return;
		}
		...
		//LOG_ERROR("[EntityRegistry] Could not remove entity with id: {} to group {}. Group does not include this entity.", entity, group_name);
	}

	std::vector<entity_desc> get_entity_group(const std::string& group_name) const{
		if (!_entity_groups.contains(group_name)){
			//LOG_ERROR("[EntityRegistry] Could not get group with name: {}. Group does not exists.", group_name);
	...
	size_t entity_to_index(uint64_t entity) const{
		...
		//LOG_ERROR("[EntityRegistry] Could not get index for entity with id: {}.", entity);
		return -1;
	}

	uint64_t index_to_entity(size_t index){
		if (index > _entities.size() || index < 0){
			//LOG_ERROR("[EntityRegistry] Could not get entity for index: {}. Index is out of bounds.", index);
			return 0;
		}
	...
	std::vector<entity_desc>& view_exclusively(){
		...
		//LOG_ERROR("[EntityRegistry] Could not process view.");
